"""
Multi-Missile Real-time DWTA Analysis Simulator - Qt/PyQtGraph GUI Version
============================================================
"""
...
# GUI is now Qt-based (no matplotlib or tkinter)
GUI_AVAILABLE = True
...
try:
    ...
    # PyQtGraph visualization (Qt-based, high-performance)
    try:
        from pyqtgraph_integration import create_pyqtgraph_visualization
...
class MultiMissileTracker:
    ...
    def __init__(self, use_mip=True, objective='MIN_DAMAGE', headless=False):
        ...
        # 성능: 탐색 O(1), 메모리 6배 절감
        self.primary_assignments = OptimizedAssignmentManager(max_threats=100, max_batteries=20)
        ...
        # 🆕 PyQtGraph visualization (완전 교체)
        self.pyqt_viz = None
        self.fig = None  # matplotlib 제거됨
        ...
        if self.headless:
            ...
            try:
                self._setup_pyqtgraph_visualization()
            except Exception as e:
                print(f"❌ PyQtGraph 초기화 실패: {e}")
                import traceback
                traceback.print_exc()
                self.pyqt_viz = None
                self.control_panel = None
        else:
            print("❌ PyQtGraph를 사용할 수 없습니다.")
            print("   설치: pip install pyqtgraph PyQt5")
            self.pyqt_viz = None
            self.control_panel = None
    
    def _setup_pyqtgraph_visualization(self):
        """PyQtGraph 시각화 + Qt 제어 패널 통합 (High Performance - 60+ FPS)"""
        print("🚀 PyQtGraph 통합 GUI 초기화 중...")

        # PyQtGraph 시각화 + Qt 제어 패널 생성 (통합)
        self.pyqt_viz = create_pyqtgraph_visualization(self)
        self.control_panel = self.pyqt_viz.control_panel  # Qt 제어 패널
        print("✅ PyQtGraph visualization + Qt control panel initialized")
        print("   - 시각화: 60+ FPS")
        print("   - 제어 패널: Qt (Tkinter 제거됨)")

    # matplotlib 관련 메서드 제거됨 - PyQtGraph로 완전 교체

    def _initialize_axes(self):
        """Deprecated: matplotlib removed"""
        pass
    ...
    def update_simulation(self):
        ...
        if self.control_panel:
            # Qt 제어 패널: update_status(time_step, stats)
            if hasattr(self.control_panel, 'update_status'):
                self.control_panel.update_status(self.current_time_step, self.stats)
    ...
    def _process_impact(self, missile_id, missile):
        ...
        if P_kill >= 0.5 and missiles_fired > 0:  # 50% 이상 확률이면 성공으로 판정
            ...
            # 🆕 PyQtGraph kill event
            if self.use_pyqtgraph and self.pyqt_viz:
                self.pyqt_viz.add_kill_event(self, missile_id)
            ...
            if self.shoot_look_shoot_enabled and current_attempt < self.max_engagement_attempts and missiles_fired > 0:
                ...
                if missile['target_asset'] == 'EMPTY_AREA':
                    ...
                    # 🆕 PyQtGraph impact event
                    if self.use_pyqtgraph and self.pyqt_viz:
                        self.pyqt_viz.add_impact_event(self, missile_id)
    ...
    def update_display(self):
        """Update the display - PyQtGraph only (60+ FPS)."""
        if self.pyqt_viz:
            try:
                self.pyqt_viz.update_display(self)
            except Exception as e:
                print(f"❌ PyQtGraph visualization error: {e}")
                import traceback
                traceback.print_exc()
    ...
    def finalize_simulation(self):
        ...
        if self.control_panel and self.last_objective_value is not None:
            # Tkinter 제어 패널 (legacy)
            if hasattr(self.control_panel, 'objective_val_var'):
                self.control_panel.objective_val_var.set(f"목적함수: {self.last_objective_value:.2f}")
            # Qt 제어 패널
            elif hasattr(self.control_panel, 'log_message'):
                self.control_panel.log_message(
                    f"📊 Final Objective Value: {self.last_objective_value:.2f}",
                    "INFO"
                )
