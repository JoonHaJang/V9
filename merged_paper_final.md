...
목적함수(식 (1))는 각 탄도미사일의 위험도에 방어 실패 확률을 곱한 기대 손실의 합을 최소화한다. 방어 실패 확률은 병렬신뢰도 개념을 적용하여, 상·하층 살보 격추확률($P_u$, $P_l$)에 교전창 품질 계수($K_{uj}$, $K_{lj}$)를 곱한 유효 격추확률을 반영한다. $K_{ij}$는 매 최적화 시점마다 위협의 현재 위치·속도로부터 계산되는 고정 파라미터이며(이하 이동시간지평 방식의 각 타임스텝), 상세 산출 방법은 아래 ◦K-factor 모형에서 서술한다.
