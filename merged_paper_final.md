...
**K-factor 파라미터 강건성 실험.** 가중치 $c_1, c_2, c_3, c_4$가 전문가 판단에 기반한 휴리스틱 값임을 감안하여, 각 가중치를 기준값 대비 $\pm 20\%$ 범위에서 단독 변화시키는 민감도 분석을 수행하였다(8개 변동 조건 + 기준 조건, 각 3회 반복). SMALL\_3(3발), MEDIUM\_10(10발), MEDIUM\_20(20발) 시나리오에 대해 요격률과 목적함수 값을 측정한 결과, 요격률 편차는 모든 조건에서 $0\,\text{pp}$(전 시나리오 100% 유지)로 운영적 의사결정 결과는 가중치 변동에 완전히 강건하였다. 목적함수(기댓값 손실) 편차는 $c_1$(시간적)이 최대 5.7\%, $c_4$(환경적)가 최대 13.5\%인 데 비해, $c_2$(기하학적)와 $c_3$(운동학적)는 최대 27.4\%의 수치적 변동을 보였다. 이는 K-factor의 기하·운동학 요소가 교전 품질 평가에 상대적으로 큰 기여를 하며, 해당 가중치 설정이 기댓값 손실의 정량적 크기에 영향을 미침을 의미한다. 그러나 실제 요격 할당 결과(요격률)는 가중치 선택에 독립적으로 유지되었으므로, 본 모형의 운용 타당성은 확보된 것으로 판단한다.
