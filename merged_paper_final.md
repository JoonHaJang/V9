...
목적함수(식 (1))는 각 탄도미사일의 위험도에 방어 실패 확률(상·하층 모두 요격 실패할 확률)을 곱한 기댓손실의 합을 최소화한다. 방어 실패 확률은 병렬신뢰도 개념을 적용하여 상·하층 살보 격추확률($P_u$, $P_l$)에 교전창 품질 계수($K_{uj}$, $K_{lj}$)를 곱한 유효 격추확률을 반영한다. 목적함수는 이진 변수 $x_{uj},\,x_{lj} \in \{0,1\}$와 고정 계수 $c_{ij} = K_{ij} \cdot P_i$의 다항곱 $\prod_i(1 - c_{ij} \cdot x_{ij})$ 구조로 인해 비선형·비볼록(Non-convex) 형태를 가지며, 이것이 3.3절 선형화의 근거이다.

동적 할당을 위해 결정변수 $x_{uj/lj}(t)$는 교전가능 시간창 포함 여부를 나타내는 이진변수로 정의되며, Rolling Horizon 방식으로 매 시점 목적함수를 재계산한다. 식 (2), (3)은 탄도미사일 1개당 상·하층 각각 최대 1개 방어체계를 할당하고, 식 (4), (5)는 유도탄 재고 제약을 명시한다. 식 (6), (7)은 레이다의 물리적 한계[1]를 반영한 동시 교전능력 제약으로, 특정 시각 $\theta$에서 유효한 교전창의 합이 포대 동시 교전능력($C_u / C_l = 6$)을 초과할 수 없다.
...
식 (1)의 목적함수에서 $K_{ij}$는 3.1절에서 설명한 바와 같이 **매 호출 시 계산되는 고정 파라미터**다. 따라서 $c_{ij} = K_{ij} \cdot P_i$는 상수이며, 단일 방어체계의 요격 기여도 $w_{ij} = c_{ij} \cdot x_{ij}$는 이진 변수의 선형 함수가 된다 — **여기까지는 선형이다**.

**그러나 다층 방어 생존확률의 곱구조는 McCormick 선형화를 요구한다.**

목적함수의 핵심은 위협 $j$에 대한 방어 실패 확률:

$$S_j = \prod_{u \in U}(1 - c_{uj}\,x_{uj}) \cdot \prod_{l \in L}(1 - c_{lj}\,x_{lj})$$

이 식을 직접 LP로 다룰 수 없는 이유를 3포대 상층($|U|=3$) 예시로 살펴보자. 상층 생존확률 $S_j^U$를 순차적으로 전개하면:

$$z_1 = 1 - c_{1j}\,x_{1j}$$

$$z_2 = z_1 \cdot (1 - c_{2j}\,x_{2j})$$

$$z_3 = z_2 \cdot (1 - c_{3j}\,x_{3j}) = S_j^U$$

$z_1$은 $x_{1j}$의 선형 함수이므로 문제없다. 그러나 $z_2$는 **연속 변수** $z_1$과 **연속 변수** $(1 - c_{2j}\,x_{2j})$의 곱 — 이것이 **Bilinear Term**이다. $z_3$도 마찬가지다.

**구체적 수치 예시.** $c_{1j}=0.880$, $c_{2j}=0.850$, $c_{3j}=0.880$ (L-SAM 3포대, $P_u=0.9775$, $K_{ij}\approx0.90$)이고 할당이 $x_{1j}=1,\,x_{2j}=0,\,x_{3j}=1$이라 하면:

$$z_1 = 1 - 0.880 \times 1 = 0.120$$

$$z_2 = 0.120 \times (1 - 0.850 \times 0) = 0.120 \times 1.000 = 0.120$$

$$z_3 = 0.120 \times (1 - 0.880 \times 1) = 0.120 \times 0.120 = 0.0144$$

즉 상층 3포대 중 2개가 동일 위협에 교전하면 상층 생존확률이 1.44%로 감소한다. LP Relaxation에서는 $x_{ij} \in [0,1]$로 완화되므로 $z_k$와 $(1 - c_{(k+1)j}\,x_{(k+1)j})$의 곱이 비볼록(Non-convex) 구조를 형성하여 하한이 극도로 느슨해진다. **McCormick 선형화는 이 각 단계의 Bilinear Term을 4개 선형 부등식으로 등가 변환**하여 문제를 표준 MILP로 전환한다.

요약: $K_{ij}$가 파라미터여서 $w_{ij} = c_{ij}\,x_{ij}$가 선형이 되더라도, Binary Tree 분해의 각 단계 $z_{k+1} = z_k \cdot (1-w_{(k+1)j})$에서 연속 변수끼리의 곱 — Bilinear Term — 이 발생하므로 McCormick 선형화는 여전히 필수적이다. IPOPT·BARON 등 비선형 솔버를 직접 적용하면 15개 위협 기준 30초 이상 소요[15]로 실시간 운용이 불가능하다.
