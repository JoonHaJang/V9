...
| $K_{ij}$ | 방어체계 $i$와 탄도미사일 $j$ 쌍에 대한 교전창 품질 계수 (초기값은 시뮬레이션 시작 전 LUT에 사전 계산되어 저장; 매 롤링 호라이즌 호출 시 위협의 현재 위치·속도 변화에 따라 재계산되어 갱신됨; MILP 내 고정 파라미터로 직접 사용) |
...
- **사전 계산 단계 (전처리 및 매 호출 시 갱신)**: 시뮬레이션 시작 전 초기 $K_{ij}$ 값을 모든 (방어체계, 위협) 쌍에 대해 계산하여 LUT에 저장한다. 이후 **매 롤링 호라이즌 호출 시마다** 위협의 현재 위치(탄도 궤적 상 갱신 좌표)를 기반으로 $K_{ij}$를 재계산하고 LUT를 갱신한다. 솔버 호출 시 최신 LUT 값을 $O(1)$로 조회하여 연산 비용을 제거한다.

- **최적화 단계 (MILP 내)**: $K_{ij}$는 매 호출 시 LUT에서 조회한 값을 **고정 파라미터**로 사용한다. 솔버는 $K_{ij}$를 최적화 대상으로 취급하지 않으며, $K_{ij} \cdot P_i$는 단일 상수 계수 $c_{ij}$로 사전 계산되어 목적함수 및 제약식에 대입된다.
...
목적함수(식 (1))는 각 탄도미사일의 위험도에 방어 실패 확률(상·하층 모두 요격 실패할 확률)을 곱한 기댓손실의 합을 최소화한다. 방어 실패 확률은 병렬신뢰도 개념을 적용하여, 상·하층 방어체계의 살보 격추확률($P_u$, $P_l$)에 교전창 품질 계수($K_{uj}$, $K_{lj}$)를 곱한 유효 격추확률을 각각 반영한다. 특히 목적함수는 이진 변수 $x_{ij} \in \{0,1\}$와 고정 계수 $c_{ij} = K_{ij} \cdot P_i$의 곱 $\prod_i(1 - c_{ij} \cdot x_{ij})$ 형태의 다항곱(Multilinear Product)을 포함하여 비선형·비볼록(Non-convex) 구조를 가지며, 이것이 3.3절 선형화의 근거가 된다.
...
본 모형에서 $K_{ij}$는 3.1절에서 설명한 바와 같이 **고정 파라미터**이다. 매 롤링 호라이즌 호출 시 LUT에서 조회한 최신값이 사용되며, 솔버는 이를 최적화 대상으로 취급하지 않는다. $K_{ij} \cdot P_i$는 단일 상수 $c_{ij}$로 사전 계산되어 MILP에 대입된다. 따라서 목적함수의 비선형·비볼록 구조는 $x_{ij}$ (이진) × $c_{ij}$의 Bilinear Term이 아니라, $\prod_i(1 - c_{ij} \cdot x_{ij})$ 형태의 다항곱(Multilinear Product)에서 발생하며, 이것이 Binary Tree 분해 및 McCormick 선형화의 적용 대상이다.
