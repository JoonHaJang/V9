...
**결과 해석.** Root Node Gap은 전 시나리오에 걸쳐 0.0000–0.0053% 범위로 측정되었으며, 모든 인스턴스에서 B\&B가 루트 노드에서 실질적인 분기 없이 종료되었다. STRESS_100은 within-capacity 구간(활성 위협 ≤ 45개)에서 avg 0.0053%, max 0.0524%로 측정되어 다른 시나리오보다 다소 높으나 여전히 무시 가능한 수준이다.

여기서 $Z_{\text{LP}}^{\text{root}}$는 CBC의 루트 노드 LP 완화값에 표준 Gomory/MIP cuts을 적용한 이후의 하한이다. LARGE_40에서 순수 McCormick LP 완화(cuts 전) 하한은 $\approx 597$로 약 28.7%의 완화 잔차가 존재하지만, 루트 노드에서 22개의 cuts을 11회 통과시키면 하한이 $Z^*$와 일치한다(`Cuts at root node changed objective from 596.8 to 836.6`). 즉, McCormick 선형화와 표준 LP cuts의 결합으로 루트 노드에서 Gap이 사실상 닫히며, 추가 B\&B 분기가 불필요해진다.

소·중규모 인스턴스(≤20개 위협)에서는 cuts 없이도 LP_raw = $Z^*$가 성립하며, 이는 위에서 증명한 두 구조적 성질—(1) 단일 교전 제약 하 분수해의 열등성, (2) 정수 꼭짓점에서의 McCormick Tightness—의 직접적 결과이다. $\delta_{\text{rel}} = 1\%$의 gapRel은 허용 상한으로서, 실제 Gap이 이를 크게 하회하여 B\&B 분기가 불필요해진다.

목적함수 $Z^* = \sum_i v_i \cdot S_i^*$는 식 (1)의 최솟값으로 양수이며, 자산 가치 가중 생존확률 합계로 직접 해석된다(낮을수록 방어 효과 높음). BASELINE_15 기준 $Z^* \approx 501$은 자산 가치 합계 $\sum v_i = 9{,}900$ 대비 약 5.1%의 기대 위험 손실에 해당한다.
