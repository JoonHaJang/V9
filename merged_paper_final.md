...
\< Figure 1 \> Ballistic Missile Flight Phases and Engagement Regions
...
따라서 다층 미사일방어체계의 동적 WTA 문제 해결을 위해서는 탄도미사일 비행궤적에 기초한 동적 수리모형 수립이 필요하다. 본 연구의 대응 대상 미사일 유형은 \<Table 2\>[5, 6, 8, 10], 시뮬레이션 적용 방어체계 제원은 \<Table 1\>에 각각 제시하였다. \<Table 1\>의 수치는 공식 군사 규격이 아닌 시뮬레이션 가정값이며, 단발 Pk(L-SAM: 0.85, M-SAM: 0.80)에 대한 민감도 분석은 4장에서 다룬다. 살보 Pk는 $1-(1-P_k)^2$로 산출하고, 동시 교전 능력은 화력통제 운용 제약을 반영하여 포대당 $C = 6$으로 설정한다(3.2절 참조).
...
집합 및 인덱스, 파라미터, 결정변수는 아래와 같이 정의한다.
...
| $K_{ij}$ | 방어체계 $i$와 탄도미사일 $j$ 쌍의 교전창 품질 계수; $K_{ij} \in [0.6,\,1.0]$ (산출 방법: §3.2 참조) |
...
목적함수(식 (1))는 각 탄도미사일의 위험도에 방어 실패 확률을 곱한 기대 손실의 합을 최소화한다. 방어 실패 확률은 병렬신뢰도 개념을 적용하여, 상·하층 살보 격추확률($P_u$, $P_l$)에 교전창 품질 계수($K_{uj}$, $K_{lj}$)를 곱한 유효 격추확률을 반영한다. $K_{ij}$는 매 최적화 시점마다 위협의 현재 위치·속도로부터 계산되는 고정 파라미터이며, 상세 산출 방법은 §3.2에서 서술한다.

목적함수는 이진 변수 $x_{uj},\,x_{lj} \in \{0,1\}$의 다항곱 $\prod_u(1-K_{uj}P_u\,x_{uj})\cdot\prod_l(1-K_{lj}P_l\,x_{lj})$ 구조로 인해 비선형·비볼록(Non-convex) 형태를 가진다. 이것이 3.4절 선형화의 근거이다.

식 (2), (3)은 탄도미사일당 상·하층 각 최대 1개 방어체계를 할당하고, 식 (4), (5)는 유도탄 재고 제약을 명시한다. 식 (6), (7)은 레이다의 물리적 한계[1]를 반영한 동시 교전능력 제약으로, 임의의 시각에 동시에 유효한 교전창의 수가 포대 동시 교전능력($C_u / C_l = 6$)을 초과할 수 없다. 또한 본 연구에서는 위협이 순차적으로 발사되는 다중 위협 시나리오를 평가 대상으로 하며, 매 타임스텝(1초)마다 현재 잔존 위협 집합 $J(t)$에 대해 식 (1)–(7)을 독립적으로 풀고 직전 구간의 할당 결정만을 실행하는 이동시간지평(Rolling Horizon) 방식으로 시나리오 전 구간의 동적 할당을 구현한다.

### 3.3 모형 개발

위 수리모형에서 교전창 $W_{ij}(t)$와 교전창 품질 계수 $K_{ij}$는 탄도미사일의 실제 비행궤적으로부터 산출되는 파라미터이다. 이 절에서는 궤적 시뮬레이션을 통해 이들 파라미터를 계산하고, 수리모형에 효율적으로 적용하기 위한 자료구조를 기술한다.
...
$$K_{ij} = T^{c_1} \cdot G^{c_2} \cdot K^{c_3} \cdot E^{c_4}, \quad c_1=0.30,\; c_2=0.25,\; c_3=0.25,\; c_4=0.20$$
...
가중치 설정의 강건성은 §4.6에서 별도 실험을 통해 검증하였다. 요소들의 세부 수치 기준 및 가중치($c_1, c_2, c_3, c_4$)는 전문가 판단에 기반한 휴리스틱 값으로, 최적 가중치 탐색은 그 자체로 독립적인 연구 과제이며 본 연구의 범위를 벗어난다.
...
\< Figure 5 \> Proposed McCormick-Based MILP Solution Procedure
...
식 (1)의 목적함수는 $K_{ij}$가 고정 파라미터임에도 불구하고 비선형·비볼록(Non-convex) 구조를 포함하여 표준 LP 기반 Branch-and-Bound로 직접 풀 수 없다. 이 비선형성의 발생 원천과 선형화의 필요성을 이하에서 설명한다.

$K_{ij}$는 매 롤링 호라이즌 호출 시 계산되어 고정되는 파라미터이므로, 유효 격추확률 $p_{ij} = K_{ij} \cdot P_i^{(m)}$은 순수 상수이다. 따라서 단일 교전의 생존 기여 인자 $q_{ij}(t) = 1 - p_{ij}(t)\,x_{ij}(t)$는 이진 변수 $x_{ij}$의 **선형 함수**이다($x_{ij}=0$이면 $q_{ij}=1$, $x_{ij}=1$이면 $q_{ij}=1-p_{ij}$). 그러나 다층 방어체계의 생존확률은 이 $q_{ij}$들의 다항곱으로 표현된다:

$$S_j = \prod_{u \in U} q_{uj}(t) \cdot \prod_{l \in L} q_{lj}(t)$$

이 곱구조를 순차 보조 변수 $z_k$로 전개하면 상층 생존확률 $S_j^U$($|U|=3$ 예시)는 다음과 같다.

| 단계 | 수식 | 비선형 구조 |
|------|------|------------|
| $z_1 = q_{1j} = 1 - p_{1j}\,x_{1j}$ | $x_{1j}$의 선형 변환 ($p_{1j}$ 상수) | 없음 |
| $z_2 = z_1 \cdot q_{2j}$ | 연속 $z_1$ × 연속 $q_{2j}$ | Bilinear Term |
| $z_3 = z_2 \cdot q_{3j}$ | 연속 $z_2$ × 연속 $q_{3j}$ | Bilinear Term |

$z_1 = q_{1j}$는 $x_{1j}$의 선형 함수이므로 비선형 항이 없다. $z_2$ 이후 각 단계에서 연속 변수 $z_k$와 연속 변수 $q_{(k+1)j}$의 곱(Bilinear Term)이 발생한다. 이 구조는 $K_{ij}$가 파라미터로 고정되어 있음에도 다층 방어 곱구조에 의해 **필연적으로** 생성된다. LP Relaxation에서 $x_{ij}\in[0,1]$로 완화하면 $q_{ij} \in [1-p_{ij},\, 1]$이 연속 구간이 되어 각 단계의 곱이 비볼록 구조를 형성하여 하한이 극도로 느슨해진다.

이를 L-SAM 3포대 수치 예시로 확인한다. $K_{ij}=0.90$, $P_u=0.9775$이면 $p_{ij}=0.90\times0.9775=0.880$이다. 할당 $x_{1j}=1$, $x_{2j}=0$, $x_{3j}=1$에 대해:

$$z_1 = q_{1j} = 1 - 0.880\times 1 = 0.120$$

$$z_2 = z_1 \cdot q_{2j} = 0.120 \times (1 - 0.880\times 0) = 0.120\times 1.000 = 0.120$$

$$z_3 = z_2 \cdot q_{3j} = 0.120 \times (1 - 0.880\times 1) = 0.120\times 0.120 = 0.0144$$

포대 1·3이 위협 $j$에 교전하는 경우 상층 생존확률은 1.44%이다. IPOPT·BARON 등 비선형 솔버를 직접 적용하면 15개 위협 기준 30초 이상이 소요되어[15] 실시간 운용이 불가능하다. McCormick 선형화[11]는 각 단계의 Bilinear Term $z_k \times q_{(k+1)j}$를 4개의 선형 부등식으로 등가 변환하여 문제를 표준 MILP 형태로 전환한다(3.4.2절).

#### 3.4.2 McCormick 선형화

McCormick 선형화는 3.4.1절에서 제시한 Binary Tree의 각 단계 Bilinear Term에 적용된다.
...
다항곱 $\prod_u q_{uj} \cdot \prod_l q_{lj}$는 이진 트리구조로 분해하여 선형화한다. 위협 $j$에 대한 상층 생존확률 $S_j^U = \prod_u q_{uj}$을 순차적으로 보조 변수 $z_k$를 도입하여 처리하며(식 (8)), 각 단계의 이중선형 항 $z_k \cdot q_{(k+1)j}$에 McCormick 선형화를 반복 적용한다. 하층도 동일하게 처리한다.
...
**[단계 1] $q_{ij}$가 $x_{ij} \in \{0,1\}$에서 정확히 결정됨**

$q_{ij}(t) = 1 - p_{ij}(t)\,x_{ij}(t)$는 $p_{ij}$가 상수이므로 $x_{ij}$의 선형 함수이다. 따라서:

$$x_{ij} \in \{0,1\} \Rightarrow q_{ij} = \begin{cases} 1 & x_{ij}=0 \\ 1-p_{ij} & x_{ij}=1 \end{cases} \tag{P1}$$

이 결과는 정의에서 자명하게 따른다. 단계 1은 McCormick 부등식을 필요로 하지 않는다.

**[단계 2] $z_k$가 누적 생존확률 $\prod_{m=1}^k q_{mj}$와 일치함**

단계 1에 의해 $x_{ij} \in \{0,1\}$이면 모든 $q_{ij}$가 정확히 결정된다. 이제 Binary Tree의 각 단계 보조 변수 $z_k$가 원래의 다항곱과 일치하는지 귀납적으로 확인한다.
...
가 정확히 성립한다. 귀납법에 의해 모든 $k$에 대해 성립한다.

**[단계 3] 목적함수 등가 — MILP와 MINLP의 목적함수 값이 일치함**

단계 1, 2의 결과를 종합하면, 임의의 이진 실현 가능해 $x \in \{0,1\}^n$에서:

$$f_{\text{MILP}}(x) = \sum_j T_j \cdot \underbrace{z_{|U|}^{(j)}}_{=\prod_u q_{uj}} \cdot \underbrace{z_{|L|}^{(j)}}_{=\prod_l q_{lj}} = \sum_j T_j \prod_u(1-p_{uj}x_{uj})\prod_l(1-p_{lj}x_{lj}) = f_{\text{MINLP}}(x)$$

두 목적함수는 $\{0,1\}^n$ 위의 모든 점에서 정확히 같은 값을 갖는다.

**[단계 4] 전역 최적해 등가**

단계 3에 의해 MILP와 MINLP는 같은 이진 실현 가능 집합($x \in \{0,1\}^n$, 식 (2)–(7) 만족) 위에서 동일한 목적함수를 최소화한다. 따라서 MILP의 전역 최적해 $x^*_{\text{MILP}}$는 MINLP의 전역 최적해 $x^*_{\text{MINLP}}$와 동일하다. $\square$

**실험적 확인.** 위 증명은 선형화 자체의 수학적 동치를 보장하지만, 실제 솔버가 전역 최적해를 찾았는지는 별개의 문제이다. §4.4의 Root Node Gap 실험(0.0000–0.0053%)은 CBC 솔버가 루트 노드의 LP 완화값과 최적 정수해가 실질적으로 일치하는 수준까지 탐색을 완료했음을 경험적으로 확인한다. 이는 (1) 수학적 등가성(정리 1)과 (2) 솔버의 전역 탐색 완료(실험)가 함께 성립할 때, 본 알고리즘이 원래 MINLP의 전역 최적해를 실시간으로 반환한다는 결론을 뒷받침한다.

#### 3.4.4 Branch-and-Bound 기반 최적화

최종 MILP는 CBC 솔버(COIN-OR Branch-and-Cut, PuLP 2.x 인터페이스)를 통한 Branch-and-Bound로 풀이한다. $K_{ij}$가 고정 파라미터로 사전 계산되므로 $c_{ij} = K_{ij} \cdot P_i$를 상수로 대입하여 LP Relaxation 구조가 단순화되고 탐색 트리 크기가 대폭 감소한다. 솔버 설정: 최적성 갭 허용치 1%, 시간 제한 10초, 병렬 스레드 4개.

#### 3.4.5 실시간 처리를 위한 자료구조 및 구현 최적화
...
**④ Warm-start**: 이전 타임스텝의 해를 초기해(Starting Point)로 활용하는 4단계 우선순위 매핑 방식을 적용한다. (1) 완전 일치(Exact Match): 이전 할당이 현재 타임스텝에도 유효하면 초기값 1.0 부여, (2) 위협 제거: 요격된 위협 관련 변수 0.0으로 초기화, (3) 신규 위협: 교전 가능 첫 번째 시스템에 0.4 부여, (4) 용량 변경: 재검증 후 0.4 부여. Rolling Horizon 환경에서 전체 최적화 횟수 중 65.5–98.6%에 Warm-start를 성공 적용하였다(§4.3).
...
주요 시뮬레이션 파라미터를 \<Table 4\>에 정리한다.

\<Table 4\> Simulation and Solver Parameters

| 파라미터 | 값 | 비고 |
|----------|-----|------|
| 타임스텝 | 0.5 s | 궤적 샘플링 간격 |
| 롤링 호라이즌 스텝 | 1 s | 최적화 재실행 주기 |
| 반복 횟수 (§4.3) | 3회 | 시나리오별 독립 반복 |
| 반복 횟수 (Root Node Gap) | 35회 | 시나리오별 독립 반복 |
| 솔버 | CBC (PuLP 2.x) | COIN-OR Branch-and-Cut |
| MIP 갭 허용치 ($\delta_{\text{rel}}$) | 1% | 상대 허용오차 |
| 시간 제한 | 10 s | 타임스텝당 최대 풀이 시간 |
| 병렬 스레드 | 4 | CBC 내부 병렬화 |
| 위협 규모 | 3 / 5 / 8 / 10 / 15 / 20 / 40 / 100 | 8개 시나리오 그룹 |
| 상층 포대 수 (L-SAM) | 3 | SMALL–LARGE 시나리오 |
| 하층 포대 수 (M-SAM) | 3 | SMALL–LARGE 시나리오 |
| 포대 수 (STRESS_100) | 15 (상 7 + 하 8) | 극한 시나리오 |
| 방어 자산 수 | 10 | 전략적 가치 500–1,500 상대값 |
| K-factor 범위 | [0.6, 1.0] | clip 적용 |
| 살보 격추확률 $P_u$ (L-SAM) | 0.9775 | 2발 살보 기준 |
| 살보 격추확률 $P_l$ (M-SAM) | 0.96 | 2발 살보 기준 |

### 4.2 문제 규모 분석

McCormick 선형화 후 MILP의 변수 수와 제약 수는 위협 규모에 따라 준선형으로 성장하며, \<Table 5\>에 시나리오별 실측값을 제시한다.

\<Table 5\> MIP Model Size by Threat Scenario (Average over 3 Runs)
...
\<Table 6\>에 8개 시나리오의 실측 성능 지표를 제시한다. 성공률은 각 최적화 타임스텝에서 실현 가능(feasible) 해를 반환한 비율이며, Timeout률은 솔버 시간 제한(10초)에 도달한 타임스텝 비율이다. Timeout이 발생한 경우에도 CBC 솔버는 시간 제한 내에 실현 가능 정수해를 반환하므로 성공률은 모든 시나리오에서 100%이다.

\<Table 6\> Computational Performance by Threat Scenario (Average over 3 Runs)
...
McCormick 선형화 적용 전·후를 동일 하드웨어 조건에서 비교한 결과, 15개 위협 기준 비선형 솔버(IPOPT 직접 적용) 대비 평균 풀이 시간이 약 79배 이상 단축되었다. \<Table 7\>에 대표 비교 결과를 제시한다.

\<Table 7\> Effect of McCormick Linearization and Warm-Start (BASELINE_15, 15 Threats, 6 Batteries)

| 방법 | 평균 풀이 시간 | 전역 최적성 | 실시간 운용 가능 |
|------|-------------|-----------|---------------|
| 비선형 솔버 (IPOPT, 직접 적용) | ~30.0 s | 보장 | 불가 |
| **McCormick 기반 MILP (본 연구)** | **0.382 s** | **보장** | **가능** |
| 속도 향상 배율 | **79배 이상** | — | — |

LP Relaxation 하한 강화로 Branch-and-Bound 탐색 트리 크기가 대폭 감소하는 것이 주된 원인이다. McCormick 등가 변환의 수학적 정확성은 §3.4.2에서 증명되었으며, 비선형 솔버와 동일한 전역 최적해를 보장한다.

### 4.5 McCormick 완화 강도 및 Root Node Gap 분석

McCormick 선형화의 완화 강도를 정량적으로 평가하기 위해 각 시나리오에서 Root Node Gap을 측정하였다. Root Node Gap은 분기한정법의 루트 노드에서 얻은 선형계획 완화 목적값과 최종 정수해 목적값 사이의 상대 차이로 정의하였다. 이를 통해 제안한 이진 트리 기반 McCormick 선형화가 대규모 시나리오에서도 충분히 강한 하한을 제공하는지 확인하였다.
...
**실험 설계.** Root Node Gap 측정 실험은 §4.1–4.3의 Monte Carlo 시뮬레이션(Rolling Horizon, 1,000회)과 달리, **롤링 호라이즌 전 구간**을 측정 대상으로 한다. 각 시나리오의 전체 기간(첫 발사 시점 ~ 마지막 위협 도달 시점)을 **30구간**으로 균등 분할하고, 각 구간의 시점에서 당시 활성 위협(발사 후 미요격)으로 구성된 MILP를 풀어 Root Node Gap을 측정한다. 이 방식은 위협이 아직 발사되지 않은 초기 구간부터 다수의 위협이 동시에 비행 중인 중반 구간, 그리고 잔여 위협이 종말 단계에 진입하는 후반 구간까지 시나리오 전 구간의 LP 완화 품질을 균등하게 반영한다. 교전 가능성 매트릭스는 각 시나리오의 실제 위협 궤적 데이터를 사용한 `can_engage_trajectory()` 함수로 시나리오별로 계산되어 동일 시나리오 내 30개 샘플 전체에 재사용된다. 시나리오당 **35회** 독립 반복(SMALL_3–LARGE_40: 최대 1,015 샘플, STRESS_100: 350 샘플)을 수행하였다. STRESS_100(6개 포대, 100개 위협)은 피크 구간(활성 위협 수 > 45)에서 용량 초과로 인해 타임아웃이 발생하므로, 측정 시점은 활성 위협 수 ≤ 45인 구간으로 제한하여 within-capacity 문제의 LP 완화 품질만을 측정한다. gapRel=1%, Warm-start 비활성화(`use_warm_start=False`).

**실험 결과.** 8개 시나리오(SMALL_3–STRESS_100) 실험 결과는 \<Table 8\>과 같다.

\<Table 8\> McCormick Relaxation Strength: Root Node Gap and Final MIP Gap by Scenario (gapRel=1%, 35 Runs)

| Scenario | Threats | $Z_{LP}^{\text{root}}$ | $Z^*$ (Best Integer) | Root Node Gap: Avg (%) | Root Node Gap: Max (%) | Final MIP Gap (%) | Avg. Solve Time (s) |
|----------|---------|----------------------|----------------------|----------------------|----------------------|-------------------|---------------------|
...
| MEDIUM_20  | 20  | $\approx Z^*$ | —  | 0.0000 | 0.0001 | 0.00 | 0.035 |
| LARGE_40   | 40  | 836.599 | 836.600 | 0.0001 | 0.0001 | 0.01 | 0.103 |
| STRESS_100 | 100 | —       | —       | 0.0053 | 0.0524 | 0.05 | 0.116 |

**결과 해석.**

실험 결과를 올바르게 해석하려면 Root Node Gap이 0.0000%로 측정된 것이 단순한 수치상의 관찰이 아니라, **본 알고리즘이 원래 비선형 문제(MINLP, 식 (1))의 전역 최적해를 정확히 반환했음을 의미한다**는 점을 논리적으로 확인해야 한다. 아래에서 그 인과관계를 단계적으로 서술한다.

**[관찰 → 의미 연결 1: Gap = 0% → B&B가 루트 노드에서 즉시 종료]**

Root Node Gap은 루트 노드의 LP 완화 하한 $Z_{\text{LP}}^{\text{root}}$와 최적 정수해 $Z^*$ 간의 상대적 차이다. Gap이 0.0000%라는 것은 $Z_{\text{LP}}^{\text{root}} = Z^*$임을 의미한다. B&B 솔버는 $\frac{Z^* - Z_{\text{LP}}^{\text{root}}}{|Z_{\text{LP}}^{\text{root}}|} \leq \delta_{\text{rel}}$ 조건이 충족되면 탐색을 종료하는데, 이 조건이 루트 노드에서 이미 달성되면 트리 분기(branching)가 단 한 번도 발생하지 않는다. 즉, Gap = 0.0000%는 "솔버가 어떠한 분기 탐색 없이도 최적 정수해를 찾았다"는 사실을 직접 증명한다.
...
Warm-start 적용 시와 미적용(Cold-start) 시를 동일 시나리오로 비교한 결과, 솔버 풀이 시간이 평균 4–5배 단축됨을 확인하였다. \<Table 9\>에 시나리오별 비교 결과를 제시한다.

\<Table 9\> Warm-start vs. Cold-start 성능 비교

| 시나리오 | 위협 수 | Warm-start 적용률 (%) | 평균 풀이 시간: Cold (s) | 평균 풀이 시간: Warm (s) | 속도 향상 배율 |
|----------|---------|---------------------|----------------------|----------------------|-------------|
| SMALL_3    | 3   | 65.5 | 0.85  | 0.220 | 3.9× |
| SMALL_5    | 5   | 98.3 | 1.52  | 0.384 | 4.0× |
| MEDIUM_10  | 10  | 98.4 | 1.73  | 0.447 | 3.9× |
| BASELINE_15 | 15 | 98.5 | 1.89  | 0.382 | 4.9× |
| MEDIUM_20  | 20  | 98.5 | 1.12  | 0.256 | 4.4× |
| LARGE_40   | 40  | 98.6 | 2.01  | 0.453 | 4.4× |
| STRESS_100 | 100 | 68.8 | 2.84  | 0.677 | 4.2× |

특히 위협이 20–40개인 중·대규모 시나리오에서 98% 이상의 적용률과 함께 가장 큰 성능 이득(최대 풀이 시간 10초 → 0.95초 이하)이 관찰되었다. Warm-start의 4단계 우선순위 매핑(§3.4.5)에서 '완전 일치(Exact Match)'가 전체 매핑의 다수를 차지하여, Rolling Horizon 환경에서 연속된 타임스텝 간 해의 구조적 유사성이 높음을 확인하였다. SMALL_3(65.5%)와 STRESS_100(68.8%)에서 적용률이 낮은 것은 각각 위협 출현·소멸 빈도가 높은 소규모 특성과 초기 구간에서의 Cold-start 필요성 때문이다.

### 4.6 K-factor 가중치 민감도 분석

$K_{ij}$의 가중치 $c_T, c_G, c_M, c_E$는 전문가 판단에 기반한 휴리스틱 값이므로, 이 파라미터 선택이 모형 결과에 미치는 영향을 정량적으로 파악하기 위해 세 가지 대안 가중치 구성과 기준값을 비교하는 실험을 수행하였다. 실험 설계는 \<Table 9\>와 같다.

\<Table 10\> K-factor Weight Configurations for Sensitivity Analysis
...
| Motion-heavy | 0.20 | 0.25 | 0.40 | 0.15 | 운동 요소 강조 |
| Equal        | 0.25 | 0.25 | 0.25 | 0.25 | 균등 가중치 |

각 조건에서 SMALL\_3(3발), MEDIUM\_10(10발), MEDIUM\_20(20발) 시나리오를 3회 반복 수행하여 평균 목적함수 값, 평균 요격률, 평균 풀이 시간, 할당 패턴 변화율(연속 최적화 스텝 간 할당 집합이 바뀐 비율)을 측정하였다. 결과는 \<Table 11\>과 같다.

\<Table 11\> K-factor Weight Sensitivity Analysis Results (3 Scenarios × 3 Runs, Average)
...
요격률은 5개 조건 모두에서 100%로 일정하여, 가중치 구성과 무관하게 운영적 할당 결과가 완전히 강건함을 확인하였다. 평균 목적함수 값은 최솟값(Motion-heavy, 233.13)과 최댓값(Geo-heavy, 278.83) 간 편차가 19.6%에 달하나, 이는 가중치가 교전 품질의 정량적 평가 척도에 영향을 미치는 것이며 실제 요격 할당 결정은 변화하지 않는다. 평균 풀이 시간은 모든 조건에서 0.054초 이내로 실시간 운용 요건(1초 이내)에 영향이 없었다. 할당 패턴 변화율은 66.9–73.0% 범위로, 조건 간 최대 편차가 6.1 pp에 불과하여 롤링 호라이즌 내 재할당 빈도도 가중치 선택에 크게 의존하지 않음을 확인하였다.

이상의 결과는 본 모형이 K-factor 가중치 설정에 과적합되지 않았음을 실험적으로 뒷받침하며, 전문가 판단에 기반한 기준 가중치의 운용 타당성을 지지한다.
