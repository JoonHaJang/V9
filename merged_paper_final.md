...
목적함수는 이진 변수 $x_{uj},\,x_{lj} \in \{0,1\}$의 다항곱 $\prod_u(1-K_{uj}P_u\,x_{uj})\cdot\prod_l(1-K_{lj}P_l\,x_{lj})$ 구조로 인해 비선형·비볼록(Non-convex) 형태를 가진다. 이것이 3.3절 선형화의 근거이다.
...
포대 1·3이 위협 $j$에 교전하는 경우 상층 생존확률은 1.44%이다. IPOPT·BARON 등 비선형 솔버를 직접 적용하면 15개 위협 기준 30초 이상이 소요되어[15] 실시간 운용이 불가능하다. McCormick 선형화[11]는 각 단계의 Bilinear Term $z_k\times(1-w_{(k+1)j})$를 4개의 선형 부등식으로 등가 변환하여 문제를 표준 MILP 형태로 전환한다(3.3.2절).
...
그러나 B&B가 최적 정수해를 찾았다는 것만으로는 부족하다. 여기서 풀린 정수 문제(MILP)가 원래의 비선형 문제(MINLP)와 동일한 문제인지를 확인해야 한다. 이는 앞서 정리 1(§3.3.3)에서 수학적으로 증명하였다: 모든 이진 실현 가능해 $x \in \{0,1\}^n$에서 McCormick 선형화 및 Binary Tree 분해의 보조 변수들이 원래의 비선형 곱과 정확히 일치하므로($w_{ij} = x_{ij} K_{ij} P_j^{(m)}$, $z_k = \prod(1-w_{mj})$ 등호 성립), MILP와 MINLP의 목적함수 값은 $\{0,1\}^n$ 전체에서 동일하다. 따라서:
