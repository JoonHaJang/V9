...
\<Table 8\> Root Node Gap Measurement Results (Rolling Horizon, gapRel=1%, 35 Runs per Scenario)

| Scenario | Threats | Samples | Avg. Vars | Avg. Cons | Root LP Bound ($Z_{LP}^{root}$) | Best Integer Obj ($Z^*$) | Avg. Gap (%) | Max. Gap (%) | Avg. Solve Time (s) |
|----------|---------|---------|-----------|-----------|--------------------------------|--------------------------|-------------|-------------|---------------------|
| SMALL_3    | 3   | 1,015 |  34 |   74 | = $Z^*$ | — | 0.0000 | 0.0000 | 0.011 |
| SMALL_5    | 5   | 1,015 |  61 |  125 | = $Z^*$ | — | 0.0000 | 0.0000 | 0.012 |
| SMALL_8    | 8   | 1,015 |  95 |  188 | = $Z^*$ | — | 0.0000 | 0.0000 | 0.015 |
| MEDIUM_10  | 10  | 1,015 | 122 |  239 | = $Z^*$ | — | 0.0000 | 0.0000 | 0.016 |
| BASELINE_15 | 15 | 1,015 | 183 |  354 | 501.312 | 501.312 | 0.0000 | 0.0000 | 0.020 |
| MEDIUM_20  | 20  | 1,015 | 246 |  476 | ≈ $Z^*$ | — | 0.0000 | 0.0001 | 0.035 |
| LARGE_40   | 40  |   980 | 476 |  912 | 836.599 | 836.600 | 0.0001 | 0.0001 | 0.103 |
| STRESS_100 | 100 |   350 | 460 |  877 | — | — | 0.0053 | 0.0524 | 0.116 |
...
\<Table 9\> K-factor Weight Configurations for Sensitivity Analysis

| Case | $c_T$ | $c_G$ | $c_M$ | $c_E$ | 의미 |
|------|-------|-------|-------|-------|------|
| Baseline     | 0.25 | 0.30 | 0.25 | 0.20 | 본 연구 기준값 |
| Geo-heavy    | 0.20 | 0.40 | 0.25 | 0.15 | 기하 요소 강조 |
| Motion-heavy | 0.20 | 0.25 | 0.40 | 0.15 | 운동 요소 강조 |
| Equal        | 0.25 | 0.25 | 0.25 | 0.25 | 균등 가중치 |

각 조건에서 SMALL\_3(3발), MEDIUM\_10(10발), MEDIUM\_20(20발) 시나리오를 3회 반복 수행하여 평균 목적함수 값, 평균 요격률, 평균 풀이 시간, 할당 패턴 변화율(연속 최적화 스텝 간 할당 집합이 바뀐 비율)을 측정하였다. 결과는 \<Table 10\>과 같다.

\<Table 10\> K-factor Weight Sensitivity Analysis Results (3 Scenarios × 3 Runs, Average)

| Case | 요격률 (%) | 평균 목적함수 | 평균 풀이 시간 (s) | 할당 패턴 변화율 (%) |
|------|-----------|------------|-----------------|-------------------|
| Baseline     | 100.0 | 240.61 | 0.036 | 72.2 |
| Geo-heavy    | 100.0 | 244.98 | 0.018 | 74.3 |
| Motion-heavy | 100.0 | 250.60 | 0.018 | 68.4 |
| Equal        | 100.0 | 238.12 | 0.018 | 72.5 |

요격률은 모든 조건에서 100%로 일정하여, 가중치 구성과 무관하게 운영적 할당 결과가 완전히 강건함을 확인하였다. 평균 목적함수 값은 최솟값(Equal, 238.12)과 최댓값(Motion-heavy, 250.60) 간 편차가 5.2%로 소폭 변동하며, 이는 가중치가 교전 품질의 정량적 평가 척도에는 영향을 미치지만 최적 할당 결정 자체는 변화시키지 않음을 의미한다. 평균 풀이 시간은 모든 조건에서 0.018–0.036초 범위에 머물러 실시간 운용 요건(1초 이내)에 영향이 없었다. 할당 패턴 변화율은 68.4–74.3% 범위로, 조건 간 최대 편차가 5.9 pp에 불과하여 롤링 호라이즌 내 재할당 빈도도 가중치 선택에 크게 의존하지 않음을 확인하였다.
