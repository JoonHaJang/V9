...
| $K_{ij}$ | 방어체계 $i$와 탄도미사일 $j$ 쌍의 교전창 품질 계수; $K_{ij} \in [0.6,\,1.0]$ (산출 방법: 아래 ◦K-factor 모형 참조) |
...
목적함수(식 (1))는 각 탄도미사일의 위험도에 방어 실패 확률을 곱한 기대 손실의 합을 최소화한다. 방어 실패 확률은 병렬신뢰도 개념을 적용하여, 상·하층 살보 격추확률($P_u$, $P_l$)에 교전창 품질 계수($K_{uj}$, $K_{lj}$)를 곱한 유효 격추확률을 반영한다. $K_{ij}$는 매 롤링 호라이즌 호출 시 위협의 현재 위치·속도로부터 계산되는 고정 파라미터로, 상세 산출 방법은 아래 ◦K-factor 모형에서 서술한다.
...
식 (2), (3)은 탄도미사일당 상·하층 각 최대 1개 방어체계를 할당하고, 식 (4), (5)는 유도탄 재고 제약을 명시한다. 식 (6), (7)은 레이다의 물리적 한계[1]를 반영한 동시 교전능력 제약으로, 특정 시각 $\theta$에서 유효한 교전창의 합이 포대 동시 교전능력($C_u / C_l = 6$)을 초과할 수 없다. 이동시간지평(Rolling Horizon) 방식으로 매 타임스텝마다 현재 잔존 위협 전체에 대해 재계획하여 실시간 환경 변화에 적응한다.

#### ◦ K-factor 모형

$K_{ij}$는 교전 윈도우 내 방어체계의 실질적인 교전 효율을 나타내는 품질 계수로, 기하학적(Geometric)·시간적(Temporal)·운동학적(Kinematic)·환경적(Environmental) 4요소의 단순 곱으로 산출된다:
...
- **환경적 요소 $k_E = 1.0$**: 이상 기상·전자전 조건 가정.

$K_{ij}$는 시뮬레이션 시작 전 LUT에 사전 계산되어 저장되며, **매 롤링 호라이즌 호출 시마다** 위협의 현재 위치·속도 변화에 따라 재계산·갱신된다. MILP 내에서는 고정 파라미터로 사용하며, $c_{ij} = K_{ij} \cdot P_i$를 상수로 계산하여 목적함수에 대입한다.
...
$K_{ij}$는 3.1절에서 설명한 바와 같이 매 호출 시 계산되어 고정되는 파라미터다. 이 경우 단일 방어체계의 요격 기여도 $w_{ij} = K_{ij} \cdot P_i \cdot x_{ij} = c_{ij} \cdot x_{ij}$는 이진 변수 $x_{ij}$의 **선형 함수**가 된다. 그렇다면 "$w_{ij}$가 선형인데 왜 McCormick 선형화가 필요한가?"

이유는 **다층 방어의 생존확률 곱구조**에 있다. 위협 $j$에 대한 방어 실패 확률은 상·하층 포대 전체에 걸친 곱:

$$S_j = \prod_{u \in U}(1-c_{uj}\,x_{uj})\cdot\prod_{l \in L}(1-c_{lj}\,x_{lj})$$

이를 LP로 다루기 위해 상층 생존확률 $S_j^U$를 순차 보조 변수 $z_k$로 전개한다 ($|U|=3$ 예시):

| 단계 | 수식 | 구조 |
|------|------|------|
| $z_1 = 1 - c_{1j}\,x_{1j}$ | $x_{1j}$의 선형 변환 | **선형** ✓ |
| $z_2 = z_1 \cdot (1 - c_{2j}\,x_{2j})$ | 연속 변수 $z_1$ × 연속 변수 $(1-c_{2j}x_{2j})$ | **Bilinear** ✗ |
| $z_3 = z_2 \cdot (1 - c_{3j}\,x_{3j})$ | 연속 변수 $z_2$ × 연속 변수 $(1-c_{3j}x_{3j})$ | **Bilinear** ✗ |

$z_1$은 선형이지만, 두 번째 단계부터 **두 연속 변수의 곱**이 발생한다. 이것이 Bilinear Term이다. $K_{ij}$가 파라미터로 고정되어 있어도 이 곱구조는 피할 수 없다.

**수치 예시.** L-SAM 3포대, $K_{ij}\approx0.90$, $P_u=0.9775$ → $c_{ij}=K_{ij}\cdot P_u=0.90\times0.9775=0.880$. 할당 $x_{1j}=1,\;x_{2j}=0,\;x_{3j}=1$일 때:

$$z_1 = 1 - 0.880\times 1 = 0.120$$

$$z_2 = \underbrace{0.120}_{z_1} \times \underbrace{(1 - 0.880\times 0)}_{=1.000} = 0.120 \quad\leftarrow \text{포대 2 미교전}$$

$$z_3 = \underbrace{0.120}_{z_2} \times \underbrace{(1 - 0.880\times 1)}_{=0.120} = \mathbf{0.0144} \quad\leftarrow \text{상층 생존확률 1.44\%}$$

포대 1·3이 교전하여 상층 생존확률이 1.44%로 감소한 결과다. LP Relaxation에서 $x_{ij}\in[0,1]$로 완화하면 $z_k$와 $(1-c_{(k+1)j}x_{(k+1)j})$의 곱이 비볼록 구조를 형성하여 하한이 극도로 느슨해지고 최적해 탐색이 불가능해진다.

**McCormick 선형화는 각 단계의 Bilinear Term $z_k\times(1-w_{(k+1)j})$를 4개의 선형 부등식으로 등가 변환**하여 문제를 표준 MILP 형태로 전환한다(3.3.2절). IPOPT·BARON 등 비선형 솔버를 직접 적용하면 15개 위협 기준 30초 이상 소요[15]로 실시간 운용이 불가능하다.
