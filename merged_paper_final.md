...
- **기하학적 요소 $k_G$**: 배터리-위협 간 거리 $d$를 최적 교전 거리 $d_\text{opt}$ 대비 비율로 평가한다. 최적 구간($0.5\,d_\text{opt} \leq d \leq 0.7\,d_\text{opt}$)에서 $k_G = 1.0$이며, 이탈 시 선형 감소; $k_G \in [0.6,\,1.0]$.

- **시간적 요소 $k_T = k_{\text{window}} \cdot k_{\text{timing}}$**: $k_{\text{window}}$는 교전창 길이 $\Delta t$에 따라 아래와 같이 구간 정의된다:
...
체계별 파라미터: L-SAM은 $t_{\min}=12\,\text{s}$, $t_\text{opt}=35\,\text{s}$; M-SAM은 $t_{\min}=8\,\text{s}$, $t_\text{opt}=20\,\text{s}$. $k_{\text{timing}}$은 창 내 잔여 시간 비율에 따라 선형 감소; $k_{\text{timing}} \in [0.7,\,1.0]$.

- **운동학적 요소 $k_K$**: 목표의 통과 고도 $h$와 종말 속도 $v$를 최적값 대비 편차로 평가한다. $k_h = \max(0.8,\; 1.0 - 0.2 \cdot |h-30|/30)$ ($h < 5\,\text{km}$이면 0.8, $h > 80\,\text{km}$이면 0.9); $k_v = \max(0.8,\; 1.0 - 0.2 \cdot \max(0,\, v/2.0-1))$; $k_K = \min(k_h \cdot k_v,\; 1.0)$.

- **환경적 요소 $k_E = 1.0$**: 이상 기상·전자전 조건을 가정하여 고정값으로 설정한다.

산출된 $K_{ij}$는 시뮬레이션 시작 전 LUT에 사전 계산되어 저장되며, 매 최적화 시점(1초)마다 위협의 현재 위치·속도 변화에 따라 재계산·갱신된다. MILP 내에서는 고정 파라미터로 사용하며, $c_{ij} = K_{ij} \cdot P_i$를 상수로 계산하여 목적함수에 대입한다.
...
$K_{ij}$는 매 롤링 호라이즌 호출 시 계산되어 고정되는 파라미터이므로, $c_{ij} = K_{ij} \cdot P_i$는 상수이며 단일 방어체계의 요격 기여도 $w_{ij} = c_{ij} \cdot x_{ij}$는 이진 변수 $x_{ij}$의 선형 함수이다. 그러나 다층 방어체계의 생존확률은 이 $w_{ij}$들의 다항곱으로 표현된다:

$$S_j = \prod_{u \in U}(1-c_{uj}\,x_{uj})\cdot\prod_{l \in L}(1-c_{lj}\,x_{lj})$$

이 곱구조를 순차 보조 변수 $z_k$로 전개하면 상층 생존확률 $S_j^U$($|U|=3$ 예시)는 다음과 같다.

| 단계 | 수식 | 비선형 구조 |
|------|------|------------|
| $z_1 = 1 - c_{1j}\,x_{1j}$ | $x_{1j}$의 선형 변환 | 없음 |
| $z_2 = z_1 \cdot (1 - c_{2j}\,x_{2j})$ | 연속 $z_1$ × 연속 $(1-c_{2j}x_{2j})$ | Bilinear Term |
| $z_3 = z_2 \cdot (1 - c_{3j}\,x_{3j})$ | 연속 $z_2$ × 연속 $(1-c_{3j}x_{3j})$ | Bilinear Term |

$z_1$은 $x_{1j}$의 선형 함수이나, $z_2$ 이후의 각 단계에서 두 연속 변수의 곱(Bilinear Term)이 발생한다. 이 구조는 $K_{ij}$가 파라미터로 고정되어 있어도 다층 방어 곱구조에 의해 필연적으로 생성된다. LP Relaxation에서 $x_{ij}\in[0,1]$로 완화하면 각 단계의 곱이 비볼록 구조를 형성하여 하한이 극도로 느슨해진다.

이를 L-SAM 3포대 수치 예시로 확인한다. $K_{ij}=0.90$, $P_u=0.9775$이면 $c_{ij}=0.90\times0.9775=0.880$이다. 할당 $x_{1j}=1$, $x_{2j}=0$, $x_{3j}=1$에 대해:

$$z_1 = 1 - 0.880\times 1 = 0.120$$

$$z_2 = 0.120 \times (1 - 0.880\times 0) = 0.120\times 1.000 = 0.120$$

$$z_3 = 0.120 \times (1 - 0.880\times 1) = 0.120\times 0.120 = 0.0144$$

포대 1·3이 위협 $j$에 교전하는 경우 상층 생존확률은 1.44%이다. IPOPT·BARON 등 비선형 솔버를 직접 적용하면 15개 위협 기준 30초 이상이 소요되어[15] 실시간 운용이 불가능하다. McCormick 선형화[11]는 각 단계의 Bilinear Term $z_k\times(1-w_{(k+1)j})$를 4개의 선형 부등식으로 등가 변환하여 문제를 표준 MILP 형태로 전환한다(3.4.2절).

#### 3.4.2 McCormick 선형화

McCormick 선형화는 3.4.1절에서 제시한 두 수준의 Bilinear Term에 각각 적용된다.

**[1수준] $w_{ij}$ 정의**: $K_{ij}$가 고정 파라미터이므로 $c_{ij} = K_{ij} \cdot P_j^{(m)}$은 상수다. 보조 변수 $w_{ij} = c_{ij} \cdot x_{ij}$는 이진 변수의 선형 함수로, 아래 4개 부등식이 이를 LP 형식으로 표현한다[11]:
...
**등가성**: $x_{ij}=0$이면 식 (8),(10)에 의해 $w_{ij}=0$; $x_{ij}=1$이면 식 (9),(11)에 의해 $w_{ij} = K_{ij} \cdot P_j^{(m)}$. 두 경우 모두 $w_{ij} = x_{ij} \cdot K_{ij} \cdot P_j^{(m)}$과 등가이다. $K_{ij}$는 매 호출 시 계산된 고정 파라미터이므로 $c_{ij} = K_{ij} \cdot P_j^{(m)}$은 상수이며, 4개 제약은 $w_{ij} = c_{ij} \cdot x_{ij}$ 관계를 LP 형식으로 표현한 것이다[11].
...
각 단계의 $z_k$(연속) × $(1-w_{(k+1)j})$(연속, $x_{(k+1)j}$를 포함)는 **2수준 Bilinear Term**으로, McCormick 선형화를 반복 적용한다. 이 Bilinear Term은 $K_{ij}$가 고정 상수라 하더라도 $w_{ij} = c_{ij} \cdot x_{ij}$를 $z_k$에 곱하는 구조상 필연적으로 발생한다. 하층 생존 확률 $S_j^L$도 동일하게 처리한다. 최종 MIP 모형은 이진 변수($x_{ij}$), 연속 보조 변수($w_{ij}$, $z_k$)와 선형 제약만으로 구성된 표준 MILP 형태가 된다. $K_{ij}$는 고정 파라미터이므로 결정변수에 포함되지 않는다.

**정리 1 (McCormick MILP 등가성).** 식 (8)–(11) 및 Binary Tree 분해로 구성된 MILP의 최적해는 원래 MINLP(식 (1))의 전역 최적해와 동일하다.

이 정리가 왜 중요한지부터 짚고 가자. 식 (1)의 목적함수는 이진 변수 $x_{ij}$와 연속 변수 $K_{ij}$의 곱, 그리고 그 곱들의 다항곱으로 이루어진 **비선형·비볼록** 구조이다. 비선형 솔버(IPOPT, BARON)는 이를 그대로 다루지만 실시간 요건을 충족하지 못하고, 단순한 변수 고정(예: $K_{ij}$를 상수로 처리)은 최적성을 손상시킨다. McCormick 선형화는 비선형 구조를 **선형 제약으로 대체**하되, 그 대체가 원래 문제와 **수학적으로 동치**임을 아래에서 증명한다.

**증명.**

핵심 아이디어는 단순하다: 최종적으로 구하는 해 $x_{ij}$가 이진수($0$ 또는 $1$)이므로, 최적해 근방에서 선형화된 보조 변수들이 원래의 비선형 곱과 **정확히 일치**하는지를 보이면 충분하다.

**[단계 1] 1수준 — $w_{ij}$가 원래 곱 $x_{ij} \cdot K_{ij} \cdot P_j^{(m)}$과 일치함**

보조 변수 $w_{ij}$는 원래 $x_{ij} \cdot K_{ij} \cdot P_j^{(m)}$이라는 비선형 곱을 대체하기 위해 도입되었다. 이진 결정변수 $x_{ij}$가 취할 수 있는 값은 오직 $0$과 $1$이므로, 두 경우를 각각 검토한다.

*경우 1: $x_{ij} = 0$인 경우.*

식 (8)에 $x_{ij}=0$을 대입하면 $w_{ij} \geq 0$, 식 (10)에 대입하면 $w_{ij} \leq 0$이 된다. 두 부등식을 동시에 만족하는 값은 $w_{ij} = 0$뿐이다. 이는 $x_{ij} \cdot K_{ij} \cdot P_j^{(m)} = 0 \cdot K_{ij} \cdot P_j^{(m)} = 0$과 정확히 일치한다. 즉, 방어체계 $i$가 위협 $j$에 교전하지 않을 때($x_{ij}=0$) 요격 기여도는 0으로 정확히 반영된다.

*경우 2: $x_{ij} = 1$인 경우.*

식 (9)에 $x_{ij}=1$을 대입하면 $w_{ij} \geq K_{ij} \cdot P_j^{(m)}$, 식 (11)에 대입하면 $w_{ij} \leq K_{ij} \cdot P_j^{(m)}$이 된다. 상한과 하한이 동일한 값 $K_{ij} \cdot P_j^{(m)}$으로 조여지므로 $w_{ij} = K_{ij} \cdot P_j^{(m)}$이 유일하게 결정된다. 이는 $x_{ij} \cdot K_{ij} \cdot P_j^{(m)} = 1 \cdot K_{ij} \cdot P_j^{(m)}$과 정확히 일치한다. 방어체계 $i$가 위협 $j$에 교전할 때 요격 기여도가 교전창 품질과 격추확률의 곱으로 정확히 반영된다.

두 경우를 종합하면:

$$x_{ij} \in \{0,1\} \Rightarrow w_{ij} = x_{ij} \cdot K_{ij} \cdot P_j^{(m)} \tag{P1}$$

이 등호는 McCormick 부등식 (8)–(11)이 정수점에서 **등호로 활성화(tight)**되는 결과이다. LP 완화 구간 $x_{ij} \in [0,1]$에서는 일반적으로 $w_{ij}$가 실제 곱보다 느슨하게 하한을 이루지만, $x_{ij}$가 $0$ 또는 $1$로 결정되는 순간 그 느슨함이 완전히 사라진다.

**[단계 2] 2수준 — $z_k$가 누적 생존확률 $\prod_{m=1}^k(1-w_{mj})$와 일치함**

단계 1에서 $x_{ij} \in \{0,1\}$이면 모든 $w_{ij}$가 정확히 결정됨을 보였다. 이제 Binary Tree의 각 단계 보조 변수 $z_k$가 원래의 다항곱과 일치하는지 귀납적으로 확인한다.

*초기값 ($k=1$):* $z_1 = 1 - w_{1j}$는 선형이므로 단계 1의 결과에 의해 $z_1 = 1 - x_{1j} K_{1j} P_j^{(m)}$, 즉 방어체계 1의 요격 실패 확률이 정확히 표현된다.

*귀납 단계 ($k \to k+1$):* $z_k$가 $\prod_{m=1}^k(1-w_{mj})$와 일치한다고 가정하면, $z_{k+1} = z_k \cdot (1-w_{(k+1)j})$는 연속 변수 $z_k$와 연속 변수 $(1-w_{(k+1)j})$의 곱이다. 이 곱에도 McCormick 선형화를 적용하는데, 단계 1에서와 마찬가지로 $x_{(k+1)j} \in \{0,1\}$이면 $(1-w_{(k+1)j})$의 값이 정확히 결정되고, $z_k$의 값도 귀납 가정에 의해 정확히 결정되므로 McCormick 부등식이 등호로 활성화되어:

$$z_{k+1} = z_k \cdot (1-w_{(k+1)j}) = \prod_{m=1}^{k+1}(1-w_{mj}) \tag{P2}$$
...
$$f_{\text{MILP}}(x) = \sum_j T_j \cdot \underbrace{z_{|U|}^{(j)}}_{=\prod_u(1-x_{uj}K_{uj}P_u)} \cdot \underbrace{z_{|L|}^{(j)}}_{=\prod_l(1-x_{lj}K_{lj}P_l)} = f_{\text{MINLP}}(x)$$
...
McCormick 선형화 적용 전·후를 동일 하드웨어 조건에서 비교한 결과, 15개 위협 기준 비선형 솔버(IPOPT 직접 적용) 대비 평균 풀이 시간이 약 79배 이상 단축되었다. \<Table 6\>에 대표 비교 결과를 제시한다.

\<Table 6\> Solver Performance Comparison (BASELINE_15, 15 Threats, 6 Batteries)
...
**실험 결과.** 8개 시나리오(SMALL_3–STRESS_100) 실험 결과는 \<Table 5\>와 같다.

\<Table 7\> Root Node Gap Measurement Results (Rolling Horizon, gapRel=1%, 35 Runs per Scenario)

| Scenario | Threats | Samples | Avg. Vars | Avg. Cons | Avg. Gap (%) | Max. Gap (%) | Avg. Solve Time (s) |
|----------|---------|---------|-----------|-----------|-------------|-------------|---------------------|
| SMALL_3    | 3   | 1,015 |  34 |   74 | 0.0000 | 0.0000 | 0.011 |
| SMALL_5    | 5   | 1,015 |  61 |  125 | 0.0000 | 0.0000 | 0.012 |
| SMALL_8    | 8   | 1,015 |  95 |  188 | 0.0000 | 0.0000 | 0.015 |
| MEDIUM_10  | 10  | 1,015 | 122 |  239 | 0.0000 | 0.0000 | 0.016 |
| BASELINE_15 | 15 | 1,015 | 183 |  354 | 0.0000 | 0.0000 | 0.020 |
| MEDIUM_20  | 20  | 1,015 | 246 |  476 | 0.0000 | 0.0001 | 0.035 |
| LARGE_40   | 40  |   980 | 476 |  912 | 0.0001 | 0.0001 | 0.103 |
...
그러나 B&B가 최적 정수해를 찾았다는 것만으로는 부족하다. 여기서 풀린 정수 문제(MILP)가 원래의 비선형 문제(MINLP)와 동일한 문제인지를 확인해야 한다. 이는 앞서 정리 1(§3.4.3)에서 수학적으로 증명하였다: 모든 이진 실현 가능해 $x \in \{0,1\}^n$에서 McCormick 선형화 및 Binary Tree 분해의 보조 변수들이 원래의 비선형 곱과 정확히 일치하므로($w_{ij} = x_{ij} K_{ij} P_j^{(m)}$, $z_k = \prod(1-w_{mj})$ 등호 성립), MILP와 MINLP의 목적함수 값은 $\{0,1\}^n$ 전체에서 동일하다. 따라서:
...
Warm-start 적용 시와 미적용(Cold-start) 시를 동일 시나리오로 비교한 결과, 솔버 풀이 시간이 평균 4–5배 단축됨을 확인하였다. \<Table 8\>에 시나리오별 비교 결과를 제시한다.

\<Table 8\> Warm-start vs. Cold-start 성능 비교
