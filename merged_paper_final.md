...
| $\hat{K}_{ij}$ | 방어체계 $i$와 탄도미사일 $j$ 쌍에 대한 교전창 품질 계수 **명목값** (매 롤링 호라이즌 호출 시 위협 위치 갱신에 따라 재계산되어 LUT에 저장; MILP 내 $K_{ij}$ 결정변수의 경계 설정에 활용) |
...
- **사전 계산 단계 (전처리 및 매 호출 시 갱신)**: 시뮬레이션 시작 전 초기 명목값 $\hat{K}_{ij}$를 모든 (방어체계, 위협) 쌍에 대해 계산하여 LUT에 저장한다. 이후 **매 롤링 호라이즌 호출 시마다** 위협의 현재 위치(탄도 궤적 상 갱신 좌표)를 기반으로 $\hat{K}_{ij}$를 거리·창 길이 요소 중심으로 재계산하고 LUT를 갱신한다. 솔버 호출 시 최신 LUT 값을 $O(1)$로 조회하여 연산 비용을 제거한다.
...
| $K_{ij}(t)$ | 방어체계 $i$와 탄도미사일 $j$의 교전창 품질 계수 (연속 변수; $K_{ij} \in [\hat{K}_{ij}-\epsilon,\;\hat{K}_{ij}+\epsilon] \cap [0.6,\,1.0]$, $\epsilon=0.05$) |
