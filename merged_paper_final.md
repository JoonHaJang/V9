...
등가성은 다음과 같이 확인된다. $x_{ij}\in\{0,1\}$이면 $q_{ij}$의 값이 정확히 결정되고, 각 단계의 McCormick 부등식 (9)–(12)이 등호로 활성화(tight)되어 $\zeta_{k+1} = z_k \cdot q_{(k+1)j}$가 성립한다. $q_{ij} \in [1-p_{ij},\,1]$의 유계 조건에서 4개 부등식이 이중선형 항의 볼록 다각형(Convex Hull)을 정확히 표현하므로, LP 완화 하한이 강화되어 분기한정법 탐색 트리 크기가 대폭 감소하고 전역 최적해를 보장하면서도 비선형 솔버 대비 처리 속도를 최대치로 향상시킨다.

#### 3.4.3 MIP 문제 구성

식 (8)–(12)의 변환을 통해 원래 비선형 목적함수(식 (1))는 이진 변수 $x_{ij}$, 연속 보조 변수 $q_{ij}$, $z_k$와 선형 제약만으로 구성된 표준 MILP로 변환된다. 각 타임스텝의 MILP 규모는 교전 가능 쌍 수 $E$, 배터리 수 $B$, 위협 수 $J$, 계층 수 $\ell$에 의해 결정된다. 희소 변수 생성으로 이진 변수 $x_{ij}$는 $O(E)$, Binary Tree 보조 변수 $q_{ij}$·$z_k$ 및 McCormick 부등식 (9)–(12)는 각각 $O(E\cdot\ell)$, 용량·배정 제약은 $O(B+J)$로, 전체 규모는 $O(E\cdot\ell+B+J)$의 준선형(quasi-linear) 구조를 유지한다. 실측값은 §4.2 \<Table 5\>에 제시한다.
...
그러나 B&B가 최적 정수해를 찾았다는 것만으로는 부족하다. 풀린 MILP가 원래 비선형 문제(MINLP)와 동일한 문제인지를 확인해야 한다. §3.4.2에서 서술한 바와 같이, $x_{ij}\in\{0,1\}$에서 McCormick 부등식 (9)–(12)이 등호로 활성화되어 MILP와 MINLP의 목적함수 값이 $\{0,1\}^n$ 전체에서 동일하다. 따라서:
